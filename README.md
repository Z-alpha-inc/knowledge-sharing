...
## Next.js × Vercel上での仕様
サーバーコンポーネントとクライアントコンポーネント(`'use client'`)で、APIに関する挙動が異なる

(もしかしたら、GETかPOSTかの違いである可能性もあるが、おそらく上記の違いが原因だと考えられる)

### サーバーコンポーネント
サーバーコンポーネントで`page.tsx`などを作った場合、そこからAPIルートを使ってGETなどを使ってデータを取得することはできない。

**対処法**

`page.tsx`の中で直接`PrismaClient`をインポートするorそれ用の関数を作って、
`PrismaClient`を直接使用する形式でデータを取得する

### クライアントコンポーネント(`'use client'`)
クライアントコンポーネントで作ったファイルの場合は、APIルートを使ってデータを取得できる(通常通り)
